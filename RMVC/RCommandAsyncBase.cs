...
namespace RMVC
{
    public abstract class RCommandAsyncBase : RCommandBase
    {
        ...
        internal bool hasParent { get; set; } = false;

        private string? pendingTitle = null;
        private double? pendingPercent = null;
        private string? pendingMessage = null;

        internal RCommandAsyncBase()
        {
        }

        internal async Task RunInternalCoreAsync(
            RTracker rTracker,
            Func<Task> runAsync)
        {
            this.rTracker = rTracker;
            this.facade = rTracker.facade;
            ...
            double percentCap = 100)
        {
            ...
            if (rTracker != null)
            {
                ...
                await rTracker.facade.ExecuteCommandAsync(
                    command,
                    rTracker.CreateChild(command, percentCap),
                    percentCap);
            }
        }

        protected async Task<TResult> ExecuteCommandAsync<TResult>(
            RCommandAsync<TResult> command,
            double percentCap = 100)
        {
            command.hasParent = true;
            ...
            {
                // Execute command with a scaled RTracker child using the provided percentCap
                return await rTracker.facade.ExecuteCommandAsync(
                    command,
                    rTracker.CreateChild(command, percentCap),
                    percentCap);
            }
